...
export class ThesisListPage extends Component {
  ...
  toggleAllTheses = (selectAll) => {
    if (selectAll) {
      this.setState({ selectedTheses: this.state.filteredTheses.map(thesis => thesis.id) });
    } else {
      this.setState({ selectedTheses: [] });
    }
  }

  selectThesis = (thesis) => {
    console.log(this.state.selectedTheses);
    //const index = this.state.selectedTheses.indexOf(thesis);
    if (index > -1) {
      console.log("removing");
      //this.setState({ selectedTheses: this.state.selectedTheses.splice(index, 1) });
      console.log(this.state.selectedTheses);
    } else {
      console.log("adding");
      //this.setState({ selectedTheses: [...this.state.selectedTheses, thesis] });
      console.log(this.state.selectedTheses);
    }
  }
    ...
    return (
      <div>
        ...
        <ThesisListElement
          ...
          toggleAllTheses={this.toggleAllTheses}
          toggleSelect={this.selectThesis}
